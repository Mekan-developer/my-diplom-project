...
class adminController extends Controller
{
    ...
    {
        $student = student::where('barcode', $student_id)->first();
        if (!$student) {
    ...
    {
        $student = student::where('barcode', $student_id)->delete();
        if ($student) {
    ...
    // public function api_pc_update(Request $request)
    // {
    //     $this->validate($request, [
    //         'pc_ip' => 'required'
    //     ]);
    //     $update_pc_ip = Computer::where('id', $pc_id)->update([
    //         'pc_ip' => $request['pc_ip']
    //     ]);
    //     if (!$update_pc_ip) {
    ...
    {
        $update = student::where('barcode', $student_id)->update([
            'lname' => $request['lname'],
            'fname' => $request['fname'],
            // 'mname' => $request['mname'],
            'email' => $request['email'],
        ...
        return redirect('admin-student')->with($args);
    }
    ...
    public function monitoring()
    {
        ...
        $pc_num = Computer::count();
        $online = Computer::where('status', 1)->count();
        return view('admin.monitoring', compact('pcs', 'student_num', 'pc_num', 'online'));
    }
}
